...
export function ProfileEditor(props: ProfileEditorProps) {
    ...
    useEffect(() => {
        ...
        formRef.current && ($(formRef.current) as any).validate({
            ...
            submitHandler: async function (form: HTMLFormElement, e: FormEvent) {
                ...
                console.log(full_name, date_of_birth, gender, patient);
                if (full_name && date_of_birth && gender && patient) {
                    const response = profile_id
                        ? await updateProfile({ id: profile_id, full_name, date_of_birth, gender, patient })
                        : await createProfile({ full_name, date_of_birth, gender, patient });
    ...
    return (
        <form key={profile_id || "_"} ref={formRef} className="Profile Editor Page">
            ...
            <FormField label="Ngày sinh" attributes={{ name: "date_of_birth", type: "date", value: profile?.date_of_birth }} />
